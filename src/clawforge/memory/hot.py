...
class HotMemory:
    ...
    def __init__(self) -> None:
        ...
        self._hot_path = self._settings.get_memory_path() / "hot"
        self._hot_path.mkdir(parents=True, exist_ok=True)
        self._entries: dict[str, MemoryEntry] = {}
        self._load_entries()

    def _load_entries(self) -> None:
        """Load all HOT memory entries from disk."""
        for file_path in self._hot_path.glob("*.json"):
            try:
                data = json.loads(file_path.read_text())
                entry = MemoryEntry(**data)
                self._entries[entry.id] = entry
            except Exception as e:
                logger.warning("Failed to load HOT entry", file=file_path.name, error=str(e))

        logger.debug("HOT memory loaded", entries=len(self._entries))
    ...
    def store(self, key: str, value: Any, tags: Optional[list[str]] = None, timestamp: Optional[float] = None) -> str:
        ...
        logger.debug("HOT entry created", key=key, id=entry_id)
        return entry_id

    def put(self, key: str, value: Any, tags: Optional[list[str]] = None) -> str:
        """Store a value in HOT memory (alias for store)."""
    ...
    def delete(self, key: str) -> bool:
        ...
        for entry_id, entry in list(self._entries.items()):
            if entry.key == key:
                ...
                file_path = self._hot_path / f"{entry_id}.json"
                if file_path.exists():
                    file_path.unlink()
                logger.debug("HOT entry deleted", key=key)
                return True
        return False

    def list_keys(self) -> list[str]:
        """List all keys in HOT memory."""
    ...
    def _save_entry(self, entry: MemoryEntry) -> None:
        ...
        file_path = self._hot_path / f"{entry.id}.json"
    ...
    def prune(self, max_age_days: int = 30) -> int:
        ...
        if pruned > 0:
            logger.info("HOT memory pruned", entries=pruned)

        return pruned

    @property
    def size(self) -> int:
        """Get number of entries in HOT memory."""
